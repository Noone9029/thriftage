...
Before the first production-profile upload, the organization owner must reserve `com.thriftage.marketplace` in the business-owned Play Console and confirm signing custody. The production profile produces an AAB for Play internal/closed testing; the preview profile produces an expiring internal APK and is not a durable beta channel.
...
For the monetized Lahore beta, upload the production-profile AAB to Play internal testing first. After physical-device acceptance, promote that same eligible build to the controlled 20–30-person Lahore closed-test allowlist. Record the Play release ID, AAB SHA-256, app-signing certificate, version code, tester-list owner, rollout state, and rollback owner. The preview APK described above remains historical engineering evidence only.
