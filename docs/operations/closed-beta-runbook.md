...
Core staging is online: the Railway API at release `adba7a9de89bcf80bac86c04cf951d0787d827ca`, Vercel admin preview, Supabase Auth/Postgres/Storage/Realtime, strict CORS, deployed authorization matrix, and Android emulator login/profile/feed flow are verified. Exact-release public API smoke passes. EAS Android build `dc462b59-c9f5-4088-a38f-1a4612596e94` is an installable internal preview; the exact APK passed an authenticated staging smoke on `emulator-5554` and accepted preview OTA group `150f9366-ca54-4e65-9cea-2582a854d303` at release `edf83509126fb8dbbdd2ac3a83e1c19f2b6f4223`. The installed package passed compact, typical, and expanded emulator sizing, 200% text, bottom-navigation labels/touch bounds, keyboard dismissal, Android back, cold deep-link, and listing-detail back-navigation checks. **Do not invite testers yet.** A three-minute, 30-worker authenticated diagnostic from Railway Singapore passes every p95/p99 target with total p50 `265.98 ms`, p95 `404.46 ms`, and p99 `653.23 ms`, but still misses the strict p50 target. Railway recorded no HTTP errors, upstream errors, or application warning/error entries in the measurement window. This regional proxy does not replace the required Pakistan-origin rerun with representative beta inventory and monitoring, so the gate remains failed. A physical-device result and durable tester install path are also still missing, and legal links, Sentry, remaining provider drills, approved content, and named operational owners remain blocked. Emulator and Expo Go checks remain engineering evidence only; only the installed preview-package result counts as artifact evidence.
