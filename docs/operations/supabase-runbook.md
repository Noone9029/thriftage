...
1. Require TLS. Use the Supavisor transaction pooler for the API and a direct/session connection for migrations. Create a random password for `thriftage_runtime` out of band; do not run the API as `postgres`.
...
5. Set the runtime role login/password out of band, use its pooled TLS URL as `DATABASE_URL`, and restart the API.
6. Run `SUPABASE_MIGRATION_DATABASE_URL=<redacted TLS URL> pnpm supabase:verify-security`. The verifier reads catalog/configuration only, not application rows.
...
All Prisma-managed `public` tables are server-only. Direct client access is **No**. `anon`, `authenticated`, and `service_role` receive no table grants; RLS is enabled; only `thriftage_runtime_server_access` grants the non-login/JWT database role server access. `_prisma_migrations` is migration-role only.
