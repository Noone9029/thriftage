...
Commerce controls default off: `PAYFAST_ENABLED`, `PAYFAST_LIVE_ENABLED`, `COD_ENABLED`, `PAYOUTS_ENABLED`, and `LOCAL_COURIER_ENABLED`. Live PayFast additionally requires `COMMERCIAL_APPROVAL_REFERENCE` and `PAYFAST_MARKETPLACE_APPROVAL_REFERENCE`; enabling the production courier requires `COMMERCIAL_APPROVAL_REFERENCE` and `COURIER_AGREEMENT_REFERENCE`. `LOCAL_COURIER_SERVICE_COUNTRY_CODE` and `LOCAL_COURIER_SERVICE_CITIES` define the server-enforced service area and default to `PK` and `Lahore`. `PAYMENT_EXPIRY_MINUTES` is fixed at 15 and `MARKETPLACE_COMMISSION_BPS` is fixed at 1000 for this beta. `LAHORE_DELIVERY_FEE_MINOR` stores the approved buyer delivery charge in PKR minor units and `LAHORE_DELIVERY_RATE_VERSION` identifies the snapshotted approved rate card. `WITHHOLDING_BPS` defaults to zero and may be nonzero only with an approved `WITHHOLDING_RULE_VERSION`. `PAYOUT_ENCRYPTION_KEY` is a server-only 32-byte base64url key and is required only while payouts are enabled; production custody and rotation must be documented before activation.

PayFast merchant IDs, signing secrets, exact callback configuration, settlement credentials, bank evidence, and payout destination plaintext remain server/operator secrets. They must never use an `EXPO_PUBLIC_` or `NEXT_PUBLIC_` name. The application intentionally supplies no guessed provider implementation when the approved integration pack is absent.
